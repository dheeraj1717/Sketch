      ...
      {isAdmin && showParticipants && (
          <div className="fixed top-24 left-20 z-20 bg-white p-4 rounded-xl shadow-2xl border border-gray-100 w-72">
              <div className="flex items-center justify-between mb-4 border-b pb-2">
                  ...
                  <button onClick={() => setShowParticipants(false)} className="text-gray-400 hover:text-black">
                      <X className="w-4 h-4" />
                  </button>
              </div>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                  {participants.map((user) => (
                      <div key={user.userId} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg group">
                          <span className="text-sm font-medium text-gray-700 truncate w-32" title={user.userId}>
                  ...
                  {participants.length === 0 && <p className="text-xs text-gray-400 text-center py-4">No other users</p>}
              </div>
          </div>
      )}
